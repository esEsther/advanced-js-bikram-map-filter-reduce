...
// 1.- Dado el array numbers cuyo valor sea [4, 5, 6, 7, 8, 9, 10], crea 
// una función elevados que sea el resultado de elevar cada número a si 
// mismo.
...
//2.- Dado el array foodList con valor ['Pizza', 'Ramen', 
// 'Paella', 'Entrecot'], generar un segundo array que consiga 
...
const result3 = staff.map(objeto => {return `${objeto.name} es ${objeto.role} y le gusta ${objeto.hobbies[0]} y ${objeto.hobbies[1]}`})
// console.log(result3)
...
// que filtre los platos veganos y saque una sentencia como la 
// del ejemplo
...
const result5 = foodList2.filter(objeto => objeto.isVeggie).map(objeto => {
    if (objeto.name === 'Tempeh'){
     return `Que rico ${objeto.name} me voy a comer!`  
    } else {
      return  `Que rica ${objeto.name} me voy a comer!` 
    }
})
console.log(result5)

// 6.- Dado el array inventory, devolver un array con los 
// nombres de los elementos que valgan más de 300 euros.
...
const result6 = inventory.filter(object => object.price > 300).map(object => object.name)
/*
  [
    'TV Samsung',
    'Viaje a Cancún'
...
// 7.- Concatena todos los elementos del array con reduce para 
...
const result8 = sentenceElements.reduce((acc, str) => acc + ' ' + str)
...
const result9 = books.filter(b => b.category === 'code')
.map(b => b.price).reduce((acc, price) => acc + price, 0)
